...
contract EquilibreTvlOracle is Ownable
{
    ...
    */
    ...
    function setUWLD(address[3] memory uwl, uint8 d) public onlyOwner {
        _setUWLD(uwl, d);
    }
    function _setUWLD(address[3] memory uwl, uint8 d) internal {
        usd = uwl[0];
        wcoin = uwl[1];
        wcoinusd = uwl[2];
        usd_d = d;
    ...
    //Edit: Push & Pull
    function _pushBankOf_e_usd(address asset) public onlyOwner {
        _bankOf_e_usd.push(asset);
    }

    function _pullBankOf_e_usd(uint n) public onlyOwner
    {
    ...
    function _pushBankOf_coin(address asset) public onlyOwner {
        _bankOf_coin.push(asset);
    }

    function _pullBankOf_coin(uint n) public onlyOwner
    {
    ...
    //Edit: Push & Pull
    function _pushBankOf_t_coin_usd(address asset, address pool, uint8 dec, address lp) public onlyOwner {
    ...
    //EXPORTS
    //Self.TVL.pool2
    function pool2() public view returns (uint256){
        return _tvl_pool2_usd();
    }
    //Self.TVL.staking
    function staking() public view returns (uint256){
        return _tvl_staking_usd();
    }
    //Self.TVL.global
